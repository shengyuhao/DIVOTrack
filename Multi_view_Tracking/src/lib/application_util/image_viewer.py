    ...
    * 'ESC' : quit
    ...
    def rectangle(self, x, y, w, h, label=None):
        ...
        if label is not None:
            ...
            pt2 = pt1[0] + 10 + text_size[0][0], pt1[1] + 10 + \
                text_size[0][1]
            cv2.rectangle(self.image, pt1, pt2, self._color, -4)
            cv2.putText(self.image, label, center, cv2.FONT_HERSHEY_PLAIN,
                        1.5, (255, 255, 255), self.thickness)
    ...
    def run(self, update_fun=None):
        ...
        while not self._terminate:
            ...
            t1 = time.time()
            save_name = os.path.join(self._caption, self.view)
            if not os.path.exists(save_name):
                os.makedirs(save_name)
            print('Saving frame' +self.frame_id)
            cv2.imwrite(os.path.join(save_name, self.frame_id + '.png'), cv2.resize(self.image, self._window_shape[:2]))
            fn += 1
        self.image[:] = 0
