...
class Engine(object):
    ...
    def search_bbox(self, fid, pid, det_list):
    ...
    def run(
        ...
        if test_only:
            ...
            for data in tqdm(self.test_loader['divo']['gallery']):
                feature = self.model(data['img'].unsqueeze(0).cuda()).tolist()
                pid= int(data['impath'].split('/')[-1].split('_')[0])
                fid = int(data['impath'].split('/')[-1].split('_')[-1].split('.')[0][1:])
                seq = seq_list[int(data['impath'].split('/')[-1].split('_')[1][3:]) - 1]
                view = view_list[int(data['impath'].split('/')[-1].split('_')[1][1]) - 1]
                #det = np.loadtxt(os.path.join(res_dir, '{}_{}.txt'.format(seq, view)), delimiter=',').tolist()
                #bbox = self.search_bbox(fid, pid, det)
                img = data['impath'].split('/')[-1]
                bbox_str = boxes[img]
            ...
            np.save('../../datasets/DIVO/npy/cross_view/OSNet/osnet.npy', save_dict)            
