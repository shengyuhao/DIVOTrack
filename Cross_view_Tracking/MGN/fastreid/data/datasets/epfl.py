...
class EPFL(ImageDataset):
    ...
    def __init__(self, root='datasets', epfl=False, **kwargs):
        ...
        gallery = lambda: self.process_dir(self.gallery_dir, is_train=False) + \
                          (self.process_dir(self.extra_gallery_dir, is_train=False) if self.epfl else [])

        super(EPFL, self).__init__(train, query, gallery, **kwargs)
